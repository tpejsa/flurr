...
void SceneManager::destroyNode(FlurrHandle a_nodeHandle)
{
  if (!isInitialized())
  {
    FLURR_LOG_WARN("SceneManager not initialized!");
  ...
  const auto&& componentHandles = node->getAllComponentHandles();
  ...
  if (parentNode)
    parentNode->removeChildNode(a_nodeHandle);
...
void SceneManager::destroyNode(const std::string& a_nodeName)
{
  if (!isInitialized())
  {
    FLURR_LOG_WARN("SceneManager not initialized!");
...
{
  std::vector<FlurrHandle> nodeHandles(m_nodes.size());
...
void SceneManager::destroyAllComponents()
{
  ...
  const auto&& componentHandles = getAllComponentHandles();
  for (const FlurrHandle componentHandle : componentHandles)
    destroyComponent(componentHandle);
}
...
void SceneManager::destroyAllComponentsOfType(NodeComponentType a_componentType)
{
  ...
  const auto&& componentHandles = getAllComponentHandlesOfType(a_componentType);
  for (const FlurrHandle componentHandle : componentHandles)
    destroyComponent(componentHandle);
}
...
void SceneManager::destroyAllComponentsOfNode(FlurrHandle a_nodeHandle)
{
  ...
  for (std::size_t componentIndex = 0; componentIndex < node->getComponentCount(); ++componentIndex)
    destroyComponent(node->getComponent(componentIndex)->getComponentHandle());
}
...
{
  std::vector<FlurrHandle> componentHandles(m_components.size());
...
std::vector<FlurrHandle> SceneManager::getAllComponentHandlesOfType(NodeComponentType a_componentType)
{
  std::vector<FlurrHandle> componentHandles(m_components.size());
  auto componentHandleRange = m_componentHandlesByType.equal_range(a_componentType);
  std::for_each(componentHandleRange.first, componentHandleRange.second,
    [&componentHandles](std::unordered_multimap<NodeComponentType, FlurrHandle>::value_type& a_chKvp) {
      componentHandles.push_back(a_chKvp.second);
    });
